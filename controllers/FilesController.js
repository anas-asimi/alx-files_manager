...
export const postUpload = async (request, response) => {
  ...
  if (fileParentId !== 0) {
    const parentFile = await dbClient.db
      .collection('files')
      .findOne({ _id: new ObjectId(fileParentId) });
    if (!parentFile) {
...
export const getShow = async (request, response) => {
  const token = request.header('X-Token') || null;
  if (!token) {
    return response.status(401).send({ error: 'Unauthorized' });
  }
  ...
  const fileDocument = await dbClient.db
    .collection('files')
    .findOne({ _id: new ObjectId(idFile), userId: user._id });
  if (!fileDocument) {
...
export const getIndex = async (request, response) => {
  const token = request.header('X-Token') || null;
  if (!token) {
    return response.status(401).send({ error: 'Unauthorized' });
  }
...
export const putPublish = async (request, response) => {
  const token = request.header('X-Token') || null;
  if (!token) {
    return response.status(401).send({ error: 'Unauthorized' });
  }
  ...
  await dbClient.db
    .collection('files')
    .updateOne({ _id: new ObjectId(idFile) }, { $set: { isPublic: true } });
...
export const putUnpublish = async (request, response) => {
  const token = request.header('X-Token') || null;
  if (!token) {
    return response.status(401).send({ error: 'Unauthorized' });
  }
  ...
  await dbClient.db
    .collection('files')
    .updateOne(
      { _id: new ObjectId(idFile), userId: user._id },
      { $set: { isPublic: false } },
    );
  fileDocument = await dbClient.db
    .collection('files')
...
export const getFile = async (request, response) => {
  ...
  const fileDocument = await dbClient.db
    .collection('files')
    .findOne({ _id: new ObjectId(idFile) });
  if (!fileDocument) {
    return response.status(404).send({ error: 'Not found' });
  }
